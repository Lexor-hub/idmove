...
- `2026-06-18 — Orquestrador — Bug de campo do Rafael (print): "Erro ao reportar: column original_delivery_id of relation deliveries does not exist 42703" + admin "não vejo mais ocorrências" — DIAGNOSTICADO E CORRIGIDO via probe read-only em produção. SÃO O MESMO ENREDO, causado pelas migrations de reentrega aplicadas em prod HOJE. (1) 42703 do Rafael: o RPC report_delivery_occurrence (20260610043311) insere original_delivery_id/attempt_number/rescheduled_from_occurrence_id na nova entrega de reentrega; quando o Rafael tentou, a migration 20260501083148 (que ADD essas colunas) ainda NÃO estava aplicada → coluna inexistente → 42703. JÁ ESTÁ APLICADA AGORA (probe: select original_delivery_id,attempt_number,rescheduled_from_occurrence_id,loaded_at,loading_sequence → HTTP 200; Ricardo gerou reentregas com original_delivery_id às 04:07). Logo NÃO recorre por esse motivo. createOccurrence só chama o RPC server-side (api.ts:1655); o insert client-side legado buildRetryPayload (src/lib/delivery-attempts.ts) está SEM chamador (código morto). (2) Admin "não vejo ocorrências": as MESMAS FKs novas criaram 3 relações occurrences↔deliveries (occurrences_delivery_id_fkey, occurrences_rescheduled_delivery_id_fkey, deliveries_rescheduled_from_occurrence_id_fkey) → o embed `deliveries(...)` do getOccurrences (api.ts:1604) virou ambíguo → PostgREST PGRST201 / HTTP 300 → a lista inteira quebrava. CORREÇÃO: desambiguado para `deliveries!occurrences_delivery_id_fkey(...)`. Provado contra prod: query corrigida retorna 200 com ocorrências reais. Verificados e OK (FK única, sem ambiguidade): getRecentDeliveryEvents (delivery_events, 1043), getDeliveryReceipts (delivery_receipts, 1321), e o map interno de getDeliveries (1380, só embeda drivers). tsc/build verdes. Empresa real tem 30 ocorrências. PENDENTE: deploy na main + Rafael atualizar o PWA (build em cache pode ainda mostrar o erro antigo).`
