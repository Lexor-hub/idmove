-- Fix: trigger em auth.users (handle_new_user padrão Supabase) cria o profile
-- automaticamente com role=CLIENT e company_id=NULL antes do INSERT manual
-- do RPC create_managed_user — disparando duplicate key 23505.
--
-- Reproduzido empiricamente em 03/06/2026 via API admin: novo auth.user
-- gerado pelo RPC já tinha profile correspondente antes do INSERT manual.
--
-- Solução: trocar o INSERT em profiles por UPSERT (ON CONFLICT) — assim o
-- profile criado pelo trigger é atualizado com os valores corretos do RPC
-- (company_id, role, full_name, etc).
--
-- Por simetria e segurança, drivers e clients também viram UPSERT-like
-- (INSERT ... ON CONFLICT DO NOTHING + SELECT pra pegar id existente).
