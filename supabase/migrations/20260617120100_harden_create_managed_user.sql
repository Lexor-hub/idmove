-- Fix (blindagem): create_managed_user passa a preencher TODAS as colunas de
-- token de auth.users com '' (nunca NULL), impedindo a corrupção do GoTrue
-- corrigida em 20260617120000_fix_auth_users_null_tokens.sql.
--
-- Mudança em relação a 20260603000000_create_managed_user_upsert.sql:
--   - INSERT into auth.users agora inclui confirmation_token, recovery_token,
--     email_change, email_change_token_new, email_change_token_current,
--     phone_change, phone_change_token, reauthentication_token = '' .
--   - Sem isso, todo usuário criado pelo RPC nascia com token NULL e quebrava
--     a enumeração de usuários do GoTrue (lista de usuários no admin + login).
--
-- O restante da função é idêntico (UPSERT de profiles/drivers/clients). Como a
-- função roda numa única transação, qualquer falha nas etapas seguintes reverte
-- também o INSERT em auth.users — não fica órfão.
--
-- Operação NÃO destrutiva: apenas CREATE OR REPLACE FUNCTION.
...
begin
  ...
  insert into auth.users (
    ...
    -- colunas de token: SEMPRE '' (nunca NULL) — senão o GoTrue quebra
